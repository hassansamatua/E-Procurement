...
import { ClipboardCheck, FileText, Gavel } from 'lucide-react';
...
export default function HODDashboard() {
  ...
  const fetchData = async () => {
    try {
      const [statsRes, reqRes] = await Promise.all([
        ...
        axios.get('/api/procurement-requests?status=PENDING_HOD&limit=10'),
      ]);
  ...
  const handleApproval = async (requestId: string, action: string) => {
    try {
      await axios.patch('/api/procurement-requests', { requestId, action });
      fetchData();
    } catch (error) {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <StatsCard title="Pending Approvals" value={requests.length} icon={<ClipboardCheck size={24} />} />
